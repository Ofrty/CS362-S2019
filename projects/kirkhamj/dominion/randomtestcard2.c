...
int randTestVillage(struct scen* scen, int v)
{
	...
	if (scen->game->deckCount[curPlayer] > 0) //if any cards in deck then 1. else none.
	{
		expDiff = 1;
	}
	else
	...
	else
	{
		...
		//hand count
		if ( v >= 1) {printf("expected cur player hand count **%d**, actual **%d**\n\t- ", (preHandSize + expDiff - 1), postHandSize);} //-1 for playing this card itself
...
int main() //(int argc, char *argv[])
{
	...
	for (int i = 0; i < TEST_COUNT; i++)
	{
		...
		testRet = randTestVillage(scen, VERBOSITY);
