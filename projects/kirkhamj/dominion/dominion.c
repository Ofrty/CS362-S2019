...
int a2Adventurer(int* curDrawnTreasure, struct gameState* curState, int* curPlayer, int* curCardDrawn, int* curTempHand, int* curZ)
{
    while (*curDrawnTreasure < 2)
    {
        //if the deck is empty we need to shuffle discard and add to deck
        if (curState->deckCount[*curPlayer] <1)
        {
            shuffle(*curPlayer, curState);
        }
        ...
        *curCardDrawn = curState->hand[*curPlayer][curState->handCount[*curPlayer]-1];//top card of hand is most recently drawn card.
        ...
        {
            curTempHand[*curZ]=*curCardDrawn;
            curState->handCount[*curPlayer]--; //this should just remove the top card (the most recently drawn one).
            (*curZ)++;
        }
    }
    ...
    while((*curZ)-1>=0)
    {
        curState->discard[*curPlayer][curState->discardCount[*curPlayer]++]=(curTempHand)[*curZ-1]; // discard all cards in play that have been drawn
